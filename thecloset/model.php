...
function get_organize_list($link){
    ...
    $sql = 'SELECT  t.item_id,item_table.img,item_table.name,category_table.category_name,datediff(CURRENT_DATE(),t.created_date) as diff 
            ...
            JOIN category_table
            ON item_table.category_id = category_table.category_id
            WHERE datediff(CURRENT_DATE(),t.created_date) >= 2
            UNION
            ...
            JOIN category_table
            ON item_table.category_id = category_table.category_id
            WHERE datediff(CURRENT_DATE(),item_table.created_date) >= 2
...
function check_cart_item($link, $item_list, $amount){
    global $err_msg;
    $stock  = (int)$item_list[0]['stock'];
    $status = (int)$item_list[0]['status'];
    
    if($stock <= 0){
        $err_msg[] = '商品の在庫がありません';
    }else if($stock < (int)$amount){
        $err_msg[] = '在庫数が足りていません';
    }
